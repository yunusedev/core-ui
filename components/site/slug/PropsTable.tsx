...
export const PropsTable = ({ displayName }: { displayName: string }) => {
  const getProps = (Props as any)[displayName + "Config"] as {
    props: {
      prop: string;
      type: string;
      default: any;
        ...
        <table className="w-full table-fixed m-0">
          ...
          <tbody>
            {getProps.props.map((item, i) => (
              ...
              >
                <td
                  className={cn("p-2.5 px-3 text-muted", {
                    "rounded-bl-lg": getProps.props.length == i + 1,
                  })}
                >
                  <span className="flex items-center gap-0">
                    {item.prop}{" "}
                    {item.required && <span className="text-warning">*</span>}
                    <Tooltip content={item.description as string}>
                      <button className="hover:bg-secondary-100 ml-2 size-7 flex items-center justify-center transition rounded-md">
                ...
                <td className="flex p-2.5 max-h-32 flex flex-wrap items-center gap-1">
                  {item.type.split(" | ").map((item, i) => (
                    <span
                      className="p-0.5 px-2 text-[14px] text-foreground bg-secondary rounded-lg"
                      key={i}
                    >
                      {item}
                    </span>
                ...
                <td
                  className={cn("p-2.5 px-3 text-muted", {
                    "rounded-br-lg": getProps.props.length == i + 1,
                  })}
                >
                  <span className="p-0.5 px-2 text-[14px] text-foreground bg-secondary rounded-lg">{`${item.default}`}</span>
                </td>
              </tr>
            ))}
          </tbody>
